...
export class MozService {
   ...
   */
  private static generateAuthSignature(accessId: string, secretKey: string): { signature: string; timestamp: string } {
   ...
   */
  private static getCredentials(): { accessId: string; secretKey: string } {
  ...
  private static async makeRequest(endpoint: string, data: Record<string, unknown>): Promise<{ data: MozApiResponse | MozKeywordApiResponse | MozCompetitorApiResponse; rateLimitInfo?: MozRateLimitInfo | null }> {
    try {
      ...
      const { accessId, secretKey } = this.getCredentials();
      const { signature } = this.generateAuthSignature(accessId, secretKey);
