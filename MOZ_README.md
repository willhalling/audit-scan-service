...
# MOZ API Token (get this from MOZ API Access page)
MOZ_API_TOKEN=your_base64_encoded_api_token_here
```

### 2. Getting MOZ API Token

1. Sign up for a [MOZ account](https://moz.com/products/mozscape/access) 
2. Go to the MOZ API Access page
3. Generate your API Token (this replaces the legacy Access ID + Secret Key)
4. The token will be provided in base64 encoded format
...
| `MOZ_API_TOKEN` | Yes* | - | MOZ API Token (base64 encoded) |
...
2. **"MOZ API credentials not configured"**
   - Set `MOZ_API_TOKEN` in your environment

3. **"MOZ API Error: 401 - Unauthorized"**
   - Check your MOZ API Token is valid
