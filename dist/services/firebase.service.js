...
class MockFirebaseService {
    async saveAuditResult(auditId, data) {
        console.log(`[MOCK] Saving audit result: ${auditId}`);
    }
    async updateAuditProgress(auditId, status, current, total) {
        console.log(`[MOCK] Updating audit progress: ${auditId} - ${status} - ${current}/${total}`);
    }
    async savePageData(auditId, url, data) {
...
class FirebaseService {
    ...
    async createAudit(auditId, url, authorUid) {
        if (!this.db)
